...
layer = Layer.first_or_create :title => "Libraries", :slug => 'libraries', :icon => ''
columns = {}
...
FasterCSV.foreach(ARGV[0]) do |row|
  ...
  if (count == 1)
    ...
    place = Place.first_or_new(
      'title' =>        row[columns['LibraryName']],
      'description' =>  row[columns['OpeningTimes']],
      'lat' =>          row[columns['lat']],
      'lng' =>          row[columns['lng']],
      'address1' =>     row[columns['Throughfare']],
      'city' =>         row[columns['Posttown']],
      'postcode' =>     row[columns['Postcode']],
      'phone' =>        row[columns['Telephone']],
      'layer' =>        layer
    )
